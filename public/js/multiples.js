$(function(){
    ...
    var gridster = $(".gridster ul").gridster({
        ...
        widget_base_dimensions: [216, 162]
    }).data('gridster');
    ...
    $("#tags")
      .keyup(function(event){
        if (event.keyCode === 13) {
          ...
          var lookFor = $('#tags').val().split(',');
          lookFor = lookFor.map(function(d){return d.trim();})

          for (var i = 0; i < lookFor.length; i++) {
            if(lookFor[i]){ //string might be empty
              var pid = +(players[lookFor[i]]);

              if ($('li#player' + pid).length === 0) {
