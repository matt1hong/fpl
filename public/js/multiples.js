$(function(){
  ...
  var lookFor, name, pid;

  $("#tags")
    .keyup(function(event){
      if (event.keyCode === 13) {
        event.preventDefault();
        lookFor = $('#tags').val().split(',').map(function(d){return d.trim();})

        for (var i = 0; i < lookFor.length; i++) {
          name = lookFor[i];
          if(name){ //string might be empty
            pid = +(players[name]);
            if ($('li#player' + pid).length === 0) { // if not already there
              drawCard(pid, name);
            }
          }
        };
  ...
  function drawCard (pid, name) {
    var nameSplit, lastName, svgTitle, nowMax, nowMin;

    $.get('/fpl/?id1=' + pid, function(result) {

      gridster.add_widget('<li id="player'+ pid + '"></li>', 1, 1, 1, 1);
      $('li#player' + pid).html(function() {
        nameSplit = name.split(' ');
        lastName = nameSplit[nameSplit.length - 1];
        return result.svg + '<p>' + lastName + '</p>';
      });
